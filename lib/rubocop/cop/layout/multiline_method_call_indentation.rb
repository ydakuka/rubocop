    ...
    module Layout
      ...
      class MultilineMethodCallIndentation < Base # rubocop:disable Metrics/ClassLength
        ...
        def hash_pair_indented?(node, pair_ancestor, given_style)
          return false unless pair_ancestor && given_style == :indented
        ...
        def alignment_base_for_chained_receiver?(receiver_chain, base_receiver)
          return true if base_receiver&.hash_type?
        ...
        def first_call_alignment_node(node)
          ...
          return node if method_on_receiver_last_line?(node, base_receiver, :array, :hash)
        ...
        def method_on_receiver_last_line?(node, base_receiver, *types)
          base_receiver &&
            node.loc.dot.line == base_receiver.last_line &&
            base_receiver.type?(*types)
        end
        ...
        def handle_node_with_block(node)
          ...
          continuation_receiver = find_continuation_receiver(node)
          return continuation_receiver if continuation_receiver

          first_call = first_call_has_a_dot(node)
          base_receiver = find_base_receiver(first_call)

          # Skip alignment for parenthesized expressions like (a + b).uniq
          return if method_on_receiver_last_line?(first_call, base_receiver, :begin)

          first_call
        end

        def find_continuation_receiver(node)
          return unless node.receiver&.call_type? && node.receiver.loc&.dot

          receiver_call = node.receiver
          return unless receiver_call.receiver
          return unless receiver_call.loc.dot.line > receiver_call.receiver.last_line
