...
RSpec.describe RuboCop::Cop::Layout::FirstHashElementIndentation, :config do
  ...
  context 'when hash is right hand side in assignment' do
    ...
    it 'corrects only the key line when value starts on next line' do
      expect_offense(<<~RUBY)
        a = {
            a:
            ^^ Use 2 spaces for indentation in a hash, relative to the start of the line where the left curly brace is.
    ...
    it 'corrects key and value lines when value starts on the same line' do
      expect_offense(<<~RUBY)
        a = {
            a: {
            ^^^^ Use 2 spaces for indentation in a hash, relative to the start of the line where the left curly brace is.
    ...
    it 'corrects value lines when value starts on the same line' do
      expect_offense(<<~RUBY)
        a = {
            a: [
            ^^^^ Use 2 spaces for indentation in a hash, relative to the start of the line where the left curly brace is.
  ...
  context 'when hash is method argument' do
    context 'and arguments are surrounded by parentheses' do
      context 'and EnforcedStyle is special_inside_parentheses' do
        ...
        it 'corrects only the key line when value starts on next line in a method call' do
          expect_offense(<<~RUBY)
            func({
                     a:
                     ^^ Use 2 spaces for indentation in a hash, relative to the first position after the preceding left parenthesis.
